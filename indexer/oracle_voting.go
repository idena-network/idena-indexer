...
func detectOracleVotingsToProlong(contracts map[common.Address]struct{}, appState *appstate.AppState, head *types.Header, config *config.Config) []common.Address {
	...
	for activeVoting := range contracts {
		if !canOracleVotingBeProlonged(activeVoting, appState, head, config) {
			continue
		}
...
func canOracleVotingBeProlonged(contractAddress common.Address, appState *appstate.AppState, head *types.Header, config *config.Config) bool {
