...
func Test_ResetOracleVotingResultsAndSummaries(t *testing.T) {
	...
	statsCollector.AddOracleVotingCallVoteProof([]byte{0x3, 0x4}, pUint64(99), false)
	...
	statsCollector.BeginApplyingTx(tx, appState)
	statsCollector.AddOracleVotingCallVote(7, []byte{0x4, 0x5}, pUint64(1), 1, pUint64(2), nil, nil, nil, false)
	statsCollector.AddTxReceipt(&types.TxReceipt{Success: true, TxHash: tx.Hash(), ContractAddress: contractAddress}, appState)
	statsCollector.CompleteApplyingTx(appState)
	block.Body.Transactions = append(block.Body.Transactions, tx)
	...
	statsCollector.BeginApplyingTx(tx, appState)
	statsCollector.AddOracleVotingCallVote(8, []byte{0x4, 0x5}, pUint64(1), 1, pUint64(1), nil, nil, nil, false)
	statsCollector.AddTxReceipt(&types.TxReceipt{Success: true, TxHash: tx.Hash(), ContractAddress: contractAddress}, appState)
	statsCollector.CompleteApplyingTx(appState)
	block.Body.Transactions = append(block.Body.Transactions, tx)
	...
	statsCollector.AddOracleVotingCallVote(7, []byte{0x4, 0x5}, pUint64(2), 2, pUint64(0), &delegatee, []byte{9}, pUint64(10), false)
	statsCollector.AddTxReceipt(&types.TxReceipt{Success: true, TxHash: tx.Hash(), ContractAddress: contractAddress}, appState)
	statsCollector.CompleteApplyingTx(appState)
	block.Body.Transactions = append(block.Body.Transactions, tx)
...
func Test_ResetOracleVotingContracts(t *testing.T) {
	...
	indexData := func() {
		...
		statsCollector.BeginApplyingTx(tx, appState)
		statsCollector.AddOracleVotingCallVoteProof([]byte{0x3, 0x4}, pUint64(1), false)
		statsCollector.AddTxReceipt(&types.TxReceipt{Success: true, TxHash: tx.Hash(), ContractAddress: contractAddress2}, appState)
		statsCollector.CompleteApplyingTx(appState)
		block.Body.Transactions = append(block.Body.Transactions, tx)
		require.Nil(t, applyBlock(bus, block, appState))
		...
		statsCollector.BeginApplyingTx(tx, appState)
		statsCollector.AddOracleVotingCallVoteProof([]byte{0x4, 0x5}, pUint64(2), false)
		statsCollector.AddTxReceipt(&types.TxReceipt{Success: true, TxHash: tx.Hash(), ContractAddress: contractAddress2}, appState)
		statsCollector.CompleteApplyingTx(appState)
		block.Body.Transactions = append(block.Body.Transactions, tx)
		...
		statsCollector.BeginApplyingTx(tx, appState)
		statsCollector.AddOracleVotingCallVoteProof([]byte{0x5, 0x6}, nil, false)
		statsCollector.AddTxReceipt(&types.TxReceipt{Success: true, TxHash: tx.Hash(), ContractAddress: contractAddress2}, appState)
		statsCollector.CompleteApplyingTx(appState)
		block.Body.Transactions = append(block.Body.Transactions, tx)
		...
		statsCollector.BeginApplyingTx(tx, appState)
		statsCollector.AddOracleVotingCallVoteProof([]byte{0x6, 0x7}, pUint64(1), false)
		statsCollector.AddTxReceipt(&types.TxReceipt{Success: true, TxHash: tx.Hash(), ContractAddress: contractAddress1}, appState)
		statsCollector.CompleteApplyingTx(appState)
		block.Body.Transactions = append(block.Body.Transactions, tx)
		...
		statsCollector.BeginApplyingTx(tx, appState)
		statsCollector.AddOracleVotingCallVote(7, []byte{0x4, 0x5}, pUint64(1), 1, pUint64(0), nil, nil, nil, false)
		statsCollector.AddTxReceipt(&types.TxReceipt{Success: true, TxHash: tx.Hash(), ContractAddress: contractAddress1}, appState)
		statsCollector.CompleteApplyingTx(appState)
		block.Body.Transactions = append(block.Body.Transactions, tx)
		require.Nil(t, applyBlock(bus, block, appState))
	...
	assertions := []assertion{
		...
		},
		{
			...
			assert: func() {
				assertOracleVotingLens(t, db, &OracleVotingLens{receipts: 13, contracts: 3, ovContracts: 3, callStarts: 2,
					committees: 8, addresses: 8, sorted: 3, sortedCommittees: 5, sortedChanges: 4, sortedCommitteesChanges: 14,
		...
		},
		{
			...
			assert: func() {
				assertOracleVotingLens(t, db, &OracleVotingLens{receipts: 13, contracts: 3, ovContracts: 3, callStarts: 2,
					committees: 8, addresses: 8, sorted: 3, sortedCommittees: 5, sortedChanges: 5, sortedCommitteesChanges: 18,
					callVoteProofs: 4, callVotes: 1, callFinishes: 1, callProlongations: 1, summaries: 3, summariesChanges: 5})
				sovc, _ = testCommon.GetSortedOracleVotingContracts(db)
				require.Equal(t, 2, sovc[0].State)
		...
		},
		{
			...
			assert: func() {
				assertOracleVotingLens(t, db, &OracleVotingLens{receipts: 14, contracts: 3, ovContracts: 3, callStarts: 2,
					committees: 12, addresses: 8, sorted: 3, sortedCommittees: 5, sortedChanges: 7, sortedCommitteesChanges: 30,
		...
		},
		{
			...
			assert: func() {
				assertOracleVotingLens(t, db, &OracleVotingLens{receipts: 15, contracts: 3, ovContracts: 3, callStarts: 2,
					committees: 12, addresses: 8, sorted: 3, sortedCommittees: 5, sortedChanges: 8, sortedCommitteesChanges: 34,
		...
		},
		{
			...
			assert: func() {
				assertOracleVotingLens(t, db, &OracleVotingLens{receipts: 16, contracts: 3, ovContracts: 3, callStarts: 2,
					committees: 12, addresses: 8, sorted: 3, sortedCommittees: 3, sortedChanges: 9, sortedCommitteesChanges: 38,
