...
func deployOracleVotingContracts(t *testing.T, listener incoming.Listener, bus eventbus.Bus, startTime time.Time, addr1, addr2 common.Address) {
	...
	statsCollector.AddOracleVotingDeploy(addr1, uint64(startTime.Unix()), new(big.Int).SetUint64(23400), []byte{0x1, 0x2},
		0, 1, 2, 3, 4, 5, 6, 7, new(big.Int).SetUint64(3000), new(big.Int).SetUint64(4000), &refundRecipient, []byte{0x3, 0x4})
	...
	// Success, fact, voting min payment and hash are nil
	...
	statsCollector.AddOracleVotingDeploy(addr2, uint64(startTime.Unix()), nil, []byte{},
		0, 11, 12, 13, 14, 15, 16, 17, nil, nil, nil, nil)
	statsCollector.AddContractStake(new(big.Int).SetUint64(12300))
...
func Test_OracleVotingContractCallStart(t *testing.T) {
	...
	require.Equal(t, 45, starts[0].CommitteeSize)
	...
	require.Equal(t, 50, starts[1].CommitteeSize)
...
func Test_OracleVotingContractCallVoteProof(t *testing.T) {
	...
	tx, _ = types.SignTx(&types.Transaction{AccountNonce: 5, To: &contractAddress}, respondentKey)
	statsCollector.BeginApplyingTx(tx, appState)
	statsCollector.AddOracleVotingCallVoteProof([]byte{0x3, 0x4}, pUint64(999), 0)
	statsCollector.AddTxReceipt(&types.TxReceipt{Success: true, TxHash: tx.Hash(), ContractAddress: contractAddress}, appState)
	statsCollector.BeginTxBalanceUpdate(tx, appState)
	appState.State.SetBalance(respondentAddress, big.NewInt(2000))
	appState.State.AddBalance(contractAddress, big.NewInt(3000))
	...
	require.Equal(t, 1, summaries[0].VoteProofs)
	require.Equal(t, 0, summaries[0].Votes)
	require.Equal(t, 999, *summaries[0].SecretVotesCount)
	require.Nil(t, summaries[0].EpochWithoutGrowth)
	require.Equal(t, 3, summaries[1].ContractTxId)
	require.Equal(t, 0, summaries[1].VoteProofs)
	require.Equal(t, 0, summaries[1].Votes)
...
func Test_OracleVotingContractCallVoteProofTwice(t *testing.T) {
	...
	tx, _ = types.SignTx(&types.Transaction{AccountNonce: 6, To: &contractAddress}, respondentKey)
	statsCollector.BeginApplyingTx(tx, appState)
	statsCollector.AddOracleVotingCallVoteProof([]byte{0x4, 0x5}, pUint64(1000), 0)
	statsCollector.AddTxReceipt(&types.TxReceipt{Success: true, TxHash: tx.Hash(), ContractAddress: contractAddress}, appState)
	statsCollector.CompleteApplyingTx(appState)
	block.Body.Transactions = append(block.Body.Transactions, tx)
	require.Nil(t, applyBlock(bus, block, appState))
	...
	require.Equal(t, 1, summaries[0].VoteProofs)
	require.Equal(t, 0, summaries[0].Votes)
	require.Equal(t, 1000, *summaries[0].SecretVotesCount)
	require.Equal(t, 3, summaries[1].ContractTxId)
	require.Equal(t, 0, summaries[1].VoteProofs)
	require.Equal(t, 0, summaries[1].Votes)
	require.Nil(t, summaries[1].SecretVotesCount)
...
func Test_OracleVotingContractCallVoteProofTwice2(t *testing.T) {
	...
	tx, _ = types.SignTx(&types.Transaction{AccountNonce: 6, To: &contractAddress}, respondentKey)
	statsCollector.BeginApplyingTx(tx, appState)
	statsCollector.AddOracleVotingCallVoteProof([]byte{0x4, 0x5}, nil, 0)
	statsCollector.AddTxReceipt(&types.TxReceipt{Success: true, TxHash: tx.Hash(), ContractAddress: contractAddress}, appState)
	statsCollector.CompleteApplyingTx(appState)
	block.Body.Transactions = append(block.Body.Transactions, tx)
	require.Nil(t, applyBlock(bus, block, appState))
...
func Test_OracleVotingContractCallVote(t *testing.T) {
	...
	tx, _ := types.SignTx(&types.Transaction{AccountNonce: 4, To: &contractAddress}, respondentKey)
	statsCollector.BeginApplyingTx(tx, appState)
	statsCollector.AddOracleVotingCallVote(7, []byte{0x4, 0x5}, pUint64(1), 2, pUint64(3), nil, nil, nil, 0)
	statsCollector.AddTxReceipt(&types.TxReceipt{Success: true, TxHash: tx.Hash(), ContractAddress: contractAddress}, appState)
	statsCollector.CompleteApplyingTx(appState)
	block.Body.Transactions = append(block.Body.Transactions, tx)
	require.Nil(t, applyBlock(bus, block, appState))
	...
	require.Equal(t, byte(7), votes[0].Vote)
	require.Equal(t, []byte{0x4, 0x5}, votes[0].Salt)
	require.Equal(t, 1, *votes[0].OptionVotes)
	require.Equal(t, 2, *votes[0].OptionAllVotes)
	require.Equal(t, 3, *votes[0].SecretVotesCount)
	require.Nil(t, votes[0].Delegatee)
	require.Nil(t, votes[0].PrevPoolVote)
	require.Nil(t, votes[0].PrevOptionVotes)
	...
	require.Equal(t, 0, summaries[0].VoteProofs)
	require.Equal(t, 1, summaries[0].Votes)
	require.Equal(t, 3, *summaries[0].SecretVotesCount)
	require.Equal(t, 3, summaries[1].ContractTxId)
	require.Equal(t, 0, summaries[1].VoteProofs)
	require.Equal(t, 0, summaries[1].Votes)
	require.Nil(t, summaries[1].SecretVotesCount)
	...
	require.Equal(t, 7, results[0].Option)
	require.Equal(t, 1, results[0].Count)
	require.Equal(t, 2, *results[0].AllCount)
}
...
func Test_OracleVotingContractCallVoteWithDelegatee1(t *testing.T) {
	...
	statsCollector.AddOracleVotingCallVote(7, []byte{0x4, 0x5}, pUint64(1), 2, pUint64(3), &delegatee, nil, nil, 0)
	statsCollector.AddTxReceipt(&types.TxReceipt{Success: true, TxHash: tx.Hash(), ContractAddress: contractAddress}, appState)
	statsCollector.CompleteApplyingTx(appState)
	block.Body.Transactions = append(block.Body.Transactions, tx)
	require.Nil(t, applyBlock(bus, block, appState))
	...
	require.Equal(t, 2, *votes[0].OptionAllVotes)
	require.Equal(t, 3, *votes[0].SecretVotesCount)
	require.Equal(t, delegatee.Hex(), *votes[0].Delegatee)
	require.Nil(t, votes[0].PrevPoolVote)
	require.Nil(t, votes[0].PrevOptionVotes)
...
func Test_OracleVotingContractCallVoteWithDelegatee2(t *testing.T) {
	...
	statsCollector.BeginApplyingTx(tx, appState)
	delegatee := tests.GetRandAddr()
	statsCollector.AddOracleVotingCallVote(7, []byte{0x4, 0x5}, nil, 2, pUint64(3), &delegatee, []byte{8}, pUint64(4), 0)
	statsCollector.AddTxReceipt(&types.TxReceipt{Success: true, TxHash: tx.Hash(), ContractAddress: contractAddress}, appState)
	statsCollector.CompleteApplyingTx(appState)
	block.Body.Transactions = append(block.Body.Transactions, tx)
	require.Nil(t, applyBlock(bus, block, appState))
	...
	require.Equal(t, byte(7), votes[0].Vote)
	require.Equal(t, []byte{0x4, 0x5}, votes[0].Salt)
	require.Nil(t, votes[0].OptionVotes)
	require.Equal(t, 2, *votes[0].OptionAllVotes)
	require.Equal(t, 3, *votes[0].SecretVotesCount)
	require.Equal(t, delegatee.Hex(), *votes[0].Delegatee)
	require.Equal(t, 8, *votes[0].PrevPoolVote)
	require.Equal(t, 4, *votes[0].PrevOptionVotes)
	...
	require.Equal(t, 1, results[0].ContractTxId)
	require.Equal(t, 7, results[0].Option)
	require.Equal(t, 0, results[0].Count)
	require.Equal(t, 2, *results[0].AllCount)
	require.Equal(t, 1, results[1].ContractTxId)
	require.Equal(t, 8, results[1].Option)
	require.Equal(t, 4, results[1].Count)
	require.Equal(t, 0, *results[1].AllCount)
}
...
func Test_OracleVotingContractCallProlongation(t *testing.T) {
	...
	statsCollector.AddOracleVotingCallProlongation(&startBlock, 7890, []byte{0x4, 0x5}, 0, 999, 999, &newEpochWithoutGrowth, pUint64(9999))
	statsCollector.AddTxReceipt(&types.TxReceipt{Success: true, TxHash: tx.Hash(), ContractAddress: contractAddress}, appState)
	statsCollector.CompleteApplyingTx(appState)
	block.Body.Transactions = append(block.Body.Transactions, tx)
	require.Nil(t, applyBlock(bus, block, appState))
	...
	require.Equal(t, 7890, prolongations[0].Epoch)
	require.Equal(t, []byte{0x4, 0x5}, prolongations[0].VrfSeed)
	require.Equal(t, 2, *prolongations[0].EpochWithoutGrowth)
	require.Equal(t, 9999, *prolongations[0].ProlongVoteCount)
	...
	require.Equal(t, 2, len(summaries))
	require.Equal(t, 1, summaries[0].ContractTxId)
	require.Equal(t, 2, *summaries[0].EpochWithoutGrowth)
	require.Equal(t, 3, summaries[1].ContractTxId)
	require.Nil(t, summaries[1].EpochWithoutGrowth)
}
...
func Test_OracleVotingContractCallProlongationWithoutStartBlock(t *testing.T) {
	...
	statsCollector.BeginApplyingTx(tx, appState)
	newEpochWithoutGrowth := byte(2)
	statsCollector.AddOracleVotingCallProlongation(nil, 7890, []byte{0x4, 0x5}, 0, 999, 999, &newEpochWithoutGrowth, pUint64(9999))
	statsCollector.AddTxReceipt(&types.TxReceipt{Success: true, TxHash: tx.Hash(), ContractAddress: contractAddress}, appState)
	statsCollector.CompleteApplyingTx(appState)
	block.Body.Transactions = append(block.Body.Transactions, tx)
	require.Nil(t, applyBlock(bus, block, appState))
...
func Test_OracleVotingContractSetNewCommitteeAndSwitchToCountingState(t *testing.T) {
	...
	tx, _ = types.SignTx(&types.Transaction{AccountNonce: 5, To: &contractAddress}, respondentKey)
	statsCollector.BeginApplyingTx(tx, appState)
	newEpochWithoutGrowth := byte(0)
	statsCollector.AddOracleVotingCallProlongation(&height, 1000, []byte{0x5, 0x6}, 0, 100, 50, &newEpochWithoutGrowth, pUint64(9999))
	statsCollector.AddTxReceipt(&types.TxReceipt{Success: true, TxHash: tx.Hash(), ContractAddress: contractAddress}, appState)
...
func Test_ClearOldEpochNotVotedCommittee(t *testing.T) {
	...
	tx, _ = types.SignTx(&types.Transaction{AccountNonce: 5, To: &contractAddress}, respondentKey)
	statsCollector.BeginApplyingTx(tx, appState)
	statsCollector.AddOracleVotingCallVoteProof([]byte{0x3, 0x4}, pUint64(90), 0)
	statsCollector.AddTxReceipt(&types.TxReceipt{Success: true, TxHash: tx.Hash(), ContractAddress: contractAddress}, appState)
	statsCollector.BeginTxBalanceUpdate(tx, appState)
	appState.State.SetBalance(respondentAddress, big.NewInt(2000))
	appState.State.AddBalance(contractAddress, big.NewInt(3000))
...
func Test_OracleVotingContractDeployBigMinPaymentAndOracleRewardFund(t *testing.T) {
	...
	statsCollector.AddOracleVotingDeploy(contractAddress1, uint64(startTime.Unix()), minPayment, []byte{0x1, 0x2},
		0, 1, 2, 3, 4, 5, 6, 7, nil, oracleRewardFund, nil, nil)
	statsCollector.AddContractStake(new(big.Int).SetUint64(12300))
